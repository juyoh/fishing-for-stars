...
public record CaughtFishC2SPayload(boolean caughtChest) implements CustomPayload {
    ...
    public static void apply(CaughtFishC2SPayload packet, ServerPlayNetworking.Context context){
        ...
        if (packet.caughtChest) {
            ...
            LootWorldContext lootContextParameterSet = (new LootWorldContext.Builder(playerEntity.getWorld())).add(LootContextParameters.ORIGIN, bobber.getPos()).luck(playerEntity.getLuck()).build(LootContextTypes.CHEST);
